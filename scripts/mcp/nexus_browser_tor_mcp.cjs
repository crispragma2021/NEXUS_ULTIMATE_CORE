#!/usr/bin/env node
/**
 * NEXUS Browser MCP (Tor): Agente de Navegación Humana y Extracción Visual OMEGA
 * Totalmente anónimo a través de Tor, con evasión de detección de bots y fingerprinting.
 */
...
const proxyMesh = require('../../scripts/nexus_proxy_mesh.cjs');
...
const OUTPUT_DIR = '/tmp/nexus_browser_tor';
...
async function ensureBrowser(recordVideo = false) {
  ...
  const proxyConfig = await proxyMesh.getProxyConfig(); // Obtener configuración de proxy (con rotación si es necesario)

  if (!browser || !browser.isConnected()) {
    browser = await chromium.launch({
      ...launchOptions,
      headless: true, // Siempre headless para scripts
      proxy: proxyConfig,
      args: [
        ...launchOptions.args,
  ...
  if (recordVideo) {
    contextOptions.recordVideo = { dir: VIDEO_DIR, size: { width: 1920, height: 1080 } };
  }
...
const TOOLS = {
  browser_navigate: {
    description: 'Navega a una URL con simulación stealth de navegador real y Tor.',
    inputSchema: {
      type: 'object',
      properties: { 
        url: { type: 'string' },
  ...
  browser_resolve_captcha: {
    description: '🧬 Detecta y resuelve CAPTCHA en la página actual vía Capsolver API',
    inputSchema: {
      type: 'object',
      properties: {
        strategy: {
...
async function handleTool(name, args) {
  ...
  await proxyMesh.humanDelay(); // Añadir delay humano en cada interacción
  ...
  switch (name) {
    ...
    case 'browser_resolve_captcha': {
      ...
      if (strategy === 'evade_only' || strategy === 'evade_then_api') {
        ...
        output += `⚠️ Evasión insuficiente, CAPTCHA persiste\n`;
        if (strategy === 'evade_only') {
          return { content: [{ type: 'text', text: output + '❌ Estrategia evade_only falló' }] };
        }
      }
      ...
      output += `\n⏱️ Tiempo total: ${Date.now() - startMs}ms`;
  ...
  try {
    if (method === 'initialize') {
      ok(id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'nexus-browser-tor-stealth', version: '2.0.0' } // Nombre distinto
      });
