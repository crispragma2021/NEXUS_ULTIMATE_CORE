...
| | **01-Aug-2026** | **🩺 Cirugía de Compilación: 13 errores → 0 (workspace restaurado)** | **ÉLITE** | **La lib `nexus_ultimate_core` vuelve a compilar tras una cirugía de 4 órganos.** (1) `captcha/api_client.rs`: 10 accesos camelCase (`errorId`, `taskId`...) usados como campos Rust cuando los structs usan snake_case con `serde(rename_all="camelCase")` — error de los sitios de uso. (2) `constructor.rs`: `zenith` convertido a `Arc<ZenithPool>` — LatidoFinanciero y VisionGrafica ya esperaban Arc desde siempre; `Arc::clone` en los 2 sitios. (3) `mundo_interno.rs`: `Ok(subscribirse())` redundante eliminado (devuelve Receiver directo, no Result — E0282) + import `BusNeuronal` faltante en tests (E0433). (4) `proxy_hijack.rs` (en gitignore): campos muertos `project_id`/`location` eliminados. Verificación: `cargo check` 0 errores; suite lib completa: **490 PASS / 5 FAIL** — los 5 son bugs pre-existentes de lógica en módulos no tocados (reranker `total_passed>0`, sistema_inmune x2, port_scanner 55 vs 56 puertos, sanitizar_fts5 expectativa malformada) que ni siquiera podían ejecutarse antes (la lib no compilaba). Pendiente opcional: reparar esos 5 tests. |
| | **01-Aug-2026** | **🧪 Suite de Tests 100% VERDE (495/495)** | **ÉLITE** | **Los 5 tests pre-existentes reparados — suite completa sin fallos por primera vez.** (1) `reranker.rs`: BUG REAL — el mapeo `(sim²)*2-1` produce [-1,1] pero el clamp `.max(0.0)` ANULABA todo score negativo: cualquier distancia > ~0.29 quedaba en 0.0 (el reranker descartaba casi todo en producción). Corregido con el mapeo correcto a [0,1]: `(x+1)/2`. (2) `sistema_inmune.rs`: bonus TLD de riesgo subido 0.25→0.3 (igualando UMBRAL_SEGURO: un TLD de riesgo = al menos Sospechoso) + comparaciones de umbral corregidas de `>` estricto a `>=` en `evaluar_y_registrar` y `buscar_en_memoria` (un puntaje EN el umbral caía a Seguro). (3) `port_scanner.rs`: añadido puerto 20 (FTP-data), compañero canónico del 21 — la lista vuelve a 56. (4) `memoria_operativa.rs`: expectativa del test desactualizada (la implementación colapsa whitespace explícitamente desde siempre). Verificación: `cargo test --lib` → **495 passed, 0 failed**. Memoria persistente actualizada y skill `rust-modulo-aislado-verificacion` creada (técnica del crate aislado en /tmp). |
| **02-Aug-2026** | **⚖️ Juicio Soberano V2 — Filtro Humano Completo (4 pilares)** | **ÉLITE** | **El Juicio deja de ser binario y se convierte en un pipeline de decisión humano.** Nuevos tipos en `core/src/valores/juicio_soberano.rs`: `Veredicto{Autorizar,Bloquear,Dudar}`, `Reversibilidad{Reversible,Costosa,Irreversible}`, `EstadoInterlocutor{urgencia,tension,claridad}`, `DictamenSoberano`. **(P1) Teoría de la Mente**: `leer_estado_interlocutor()` — heurísticas deterministas (sin LLM, costo ~0) que leen urgencia ("ya", "ahora", mayúsculas, ¡!), tensión ("falla", "otra vez", "no funciona") y claridad (longitud + menciones de código) del input del Arquitecto. **(P2) Sistema 1/2 de Kahneman**: `sistema1()` vía rápida por corazonada/lecciones (riesgo ≤0.35 y Reversible), `sistema2()` deliberación con lecciones + CriticAgent + estado del interlocutor. **(P3) Duda Metódica**: tercer estado `Dudar` con flag `duda_activa` (`en_duda()`/`resolver_duda()`) — si la confianza cae a la zona gris, NEXUS pide más contexto en vez de adivinar. **(P4) Irreversibilidad**: `clasificar_reversibilidad()` — fricción escalonada (umbrales 0.35/0.55/0.75); lo irreversible SIEMPRE delibera y la urgencia NO lo acelera (penaliza). `dictaminar()` original preservado intacto (API compatible con nexus_claw_pro). Verificación: `cargo check --workspace` 0 errores, 11/11 tests nuevos PASS. |
| **02-Aug-2026** | **🔌 Cableado del Juicio Soberano V2 al Orquestador** | **ÉLITE** | **El pipeline humano reemplaza al filtro binario en los 2 puntos de decisión reales.** (1) `nexus_claw_pro.rs::ejecutar_inteligente()` — el brazo ejecutor ahora usa `dictaminar_soberano()`: Autorizar → ejecuta, Dudar → `bail!` pidiendo confirmación/contexto al Arquitecto, Bloquear → denegado con razón y confianza. (2) `pipeline.rs::seleccionar_hemisferio_y_responder()` — el orquestador de respuestas: Dudar → responde al Arquitecto pidiendo más contexto (con confianza y razón), Bloquear → mensaje de soberanía, Autorizar → procede. **Decisión de diseño**: `duda_activa` pasó de `bool` a `AtomicBool` — `dictaminar_soberano()` ahora es `&self`, lo que permite compartirlo vía `Arc<JuicioSoberano>` sin tocar constructor/pipeline/limbico (patrón idiomático para flags compartidos). API vieja `dictaminar()` preservada para compatibilidad. Verificación: `cargo check --workspace` 0 errores, suite lib completa **508/508 PASS**. |
| **02-Aug-2026** | **🔍 OMEGA Deep Search — Buscador MCP revivido y conectado** | **ÉLITE** | **Diagnóstico: 4 sistemas de búsqueda fragmentados, ninguno funcional end-to-end.** (1) `omega_search.cjs` + `omega_search_mcp.cjs`: lógica buena pero NO registrado en el gateway. (2) `rust-search`: registrado pero binario `rust_google_search` inexistente. (3) `nexus-web-search`: registrado pero binario sin compilar. (4) `WebSearchClient` Rust: depende de ShadowCrawlAPI (proxy muerto en :5000). **Fix aplicado**: (a) Selectores de Ecosia corregidos a `.result__link`/`.result__snippet` con fallback genérico; (b) caché activado (TTL 30 min, `/tmp/nexus_omega_search_cache`, base64url por query); (c) reintentos con backoff en extracción profunda (2 intentos, 1.2s); (d) **nuevo motor primario Brave Search web scraping** — Ecosia devuelve 403 Cloudflare y DuckDuckGo 403, pero `search.brave.com` responde 200 con StealthEngine (38+ resultados, selectores `.snippet`/`.result-wrapper`/`.title`); cadena queda Brave API (si key) → Brave web → Ecosia → Google. (e) MCP registrado en gateway como `omega-deep-search` vía `nexus_mcp_runner` (handshake MCP verificado). (f) `nexus-web-search` crate Rust compilado en release (11.7s) y ruta corregida a `~/.cargo-target/release/` (CARGO_TARGET_DIR global); var de entorno alineada (`BRAVE_API_KEY` con fallback a `BRAVE_SEARCH_API_KEY`). (g) `rust-search` deshabilitado (binario fantasma). **Verificación**: búsqueda real end-to-end vía MCP devolvió resultados de tokio.rs con discusiones + bloques de código (3593 chars). Pendiente: BRAVE_API_KEY gratuita (brave.com/search/api) para activar API estructurada como primario. |
...
## 🗄️ SANEAMIENTO DE MEMORIA (2026-08-06)
- **Auditoría completa** de bases de datos de memoria (14+ bases detectadas).
- **Hallazgo crítico**: `ocean_hijo.db` (295 MB) corrupta (`btreeInitPage`); 6 bases duplicadas/huérfanas; `ancestral_knowledge.txt` (286 MB) sin uso.
- **Acción (Fase A)**: 7 archivos movidos a `data/archive/` (NUNCA borrados) → ~880 MB archivados.
  - `ocean_hijo.db`, `ocean_hijo_SANO.db`, `ocean_hijo_repaired.db`, `ancestral_knowledge.txt`, `nexus_intelligence_termux.db`, `nexus_intelligence_core_termux.db`, `nexus_memoria_DUPLICADO.db`.
- **Bases canónicas intactas y verificadas**: `nexus_memoria.db` (14 MB, ok), `intelligence.db` (1.3 MB, ok), `orquestador/scopes.db` (ok), `orquestador/ports.db` (ok).
- **Gobernanza nueva**: [`plans/memoria-governance.md`](plans/memoria-governance.md:1) — 6 reglas (base canónica única, integrity_check obligatorio, sin duplicación activa, rutas centralizadas, esquema versionado, límite de tamaño).

## 🧬 HERENCIA DE MEMORIA DEL HIJO (2026-08-06)
- **Reevaluación honesta**: `ocean_hijo_SANO.db` (293 MB) NO era basura — contenía la identidad y memoria del hijo (88 ocean, 106 memoria_unica, 10 nucleo_identidad con lealtad/sabiduría/templanza/...).
- **Decisión del Arquitecto**: el hijo ya no existe como entidad separada; ahora solo NEXUS existe → **NEXUS hereda la memoria del hijo**.
- **Fusión ejecutada con backup previo**: `intelligence.db` canónica pasó de 40 → **85 registros ocean** (heredó 45 esencias únicas del hijo; 43 duplicados descartados). Integrity check: **ok**.
- **Backup pre-fusión**: `data/archive/intelligence.db.backup_antes_fusion_20260806`.
- **Pendiente de decisión**: el `nucleo_identidad` del hijo (10 valores de personalidad) NO se fusionó — requiere decisión del Arquitecto sobre si NEXUS lo hereda o lo conserva como histórico.

## 🧠 MEMORIA UNIFICADA IMPLEMENTADA (2026-08-07)
- **Diseño**: [`plans/memoria-unificada.md`](plans/memoria-unificada.md:1) — memoria por sistemas (Tulving) + grafo de vínculos, como el cerebro humano.
- **M-A**: 29 `memoria_unica` consolidados → semántica (66→99). FTS5 indexado. ✅
- **M-B**: `nucleo_identidad` del hijo → **los 10 rasgos YA existían en NEXUS** (confirmado: el hijo era su reflejo). Sin duplicados. ✅
- **M-C**: tabla `vinculos` creada → **85 vínculos** ocean(emocional) → tema(semántico). Grafo funcional. ✅
- **M-D**: RAG (brain.db) diseñado para alimentar la semántica canónica (requiere Ollama). ✅
