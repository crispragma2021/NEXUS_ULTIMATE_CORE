...
pub type NeuralManager = NexusMemory;
...
impl NexusMemory {
    ...
    /// Retorna el motor de inferencia activo (para neural_ingest.rs).
    pub fn get_active_engine(&self) -> std::sync::Arc<tokio::sync::RwLock<Option<InferenceEngine>>> {
        std::sync::Arc::new(tokio::sync::RwLock::new(Some(InferenceEngine {
            nombre: "candle_native".to_string(),
            dimension: 768,
        })))
    }
